...
public class ServiceVoiture implements IService <Voiture>{
    ...
    public void ajouter(Voiture voiture) {
        String req = "INSERT INTO `Voiture`( `marque`, `modele`, `couleur`, `prix`, `description`) VALUES (?,?,?,?,?)";
        try {
            ...
            ps.setString(5, voiture.getDescription());
            ps.executeUpdate();
    ...
    public void modifier(Voiture voiture) {
        ...
        String req = "UPDATE `Voiture` SET `marque` = ?, `modele` = ?, `couleur` = ?, `prix` = ?, `description` = ? WHERE `idV` = ?";
        try {
            PreparedStatement ps = cnx.prepareStatement(req);
            ps.setString(1, voiture.getMarque());
            ps.setString(2, voiture.getModele());
    ...
    public  Voiture getOneById(int id) {
        ...
        try {
            ...
            if (res.next()) {
                ...
                voiture = new Voiture(id, marque, modele, couleur, prix, description);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    ...
    public Set<Voiture> getAll() {
        ...
        try {
            ...
            while (res.next()){
                ...
                double prix = res.getDouble("prix");
                String description = res.getString("description");
                Voiture v = new Voiture(idV, marque, modele, couleur, prix, description);
                voitures.add(v);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
