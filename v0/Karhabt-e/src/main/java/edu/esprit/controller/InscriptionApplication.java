...
public class InscriptionApplication{
                ...
                while (!isCodeVerified) {
                    ...
                    if (result.isPresent()) {
                        ...
                        if (inputCode.equals(this.verificationCode)) {
                            isCodeVerified = true;
                            //saveCredentialsToFile(fxmail.getText(), fxpwd.getText(), fxnom.getText(), fxprenom.getText(), Integer.parseInt(fxtel.getText()), Date.valueOf(fxdate.getValue()), imagePath);
                            String hashedPassword = BCrypt.hashpw(fxpwd.getText(), BCrypt.gensalt());
                            // Now proceed with account creation, including password encryption
                           // String encryptedPassword = encrypt(fxpwd.getText());
                            serviceUser.ajouterUser(new Client(fxnom.getText(), fxprenom.getText(), Date.valueOf(fxdate.getValue()), Integer.parseInt(fxtel.getText()), fxmail.getText(), hashedPassword, imagePath));
                            Parent root = null;
                            try {
    ...
    private void saveCredentialsToFile(String email, String password, String nom, String prenom, int tel, Date date, String imagePath) {
        // Define the path to the .env file where you want to save user credentials
        String filePath = "C:/Users/RAOUF/Desktop/ProjetPI/Karhabt-e/.idea/.env"; // Update to your actual file path

        try (FileWriter fileWriter = new FileWriter(filePath, true)) {
            // Append user details to the file
            fileWriter.append("USER_EMAIL=").append(email).append("\n");
            fileWriter.append("USER_PASSWORD=").append(password).append("\n");
            fileWriter.append("USER_NOM=").append(nom).append("\n");
            fileWriter.append("USER_PRENOM=").append(prenom).append("\n");
            fileWriter.append("USER_TEL=").append(String.valueOf(tel)).append("\n");
            fileWriter.append("USER_DATE=").append(date.toString()).append("\n");
            fileWriter.append("USER_IMAGE_PATH=").append(imagePath).append("\n\n");
        } catch (IOException e) {
            e.printStackTrace();
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("File Write Error");
            alert.setContentText("Failed to write credentials to file: " + e.getMessage());
            alert.showAndWait();
        }
    }
