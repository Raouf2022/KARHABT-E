...
public class LoginApplication {
    ...
    void loginAccount(ActionEvent event) {
            ...
            ServiceUser serviceUser = new ServiceUser();
            User loggedInUser = serviceUser.getOneByEmail(email);
        if (loggedInUser != null ) {
                    if (loggedInUser.getRole().equals("Admin")) {
                        loadScene("/adminPage.fxml", event);
                    } else if (loggedInUser.getRole().equals("Client")&& BCrypt.checkpw(password, loggedInUser.getPasswd())) {
                        try {
    ...
    @FXML
    void faceID(ActionEvent event) throws IOException {
        fxLogin.getText();
        User user =  serviceUser.getOneByEmail(fxLogin.getText());
        System.out.println(user);
        FXMLLoader load = new FXMLLoader(getClass().getResource("/faceRecognition.fxml"));
        Parent root =load.load();
        FaceRecognition fr=  load.getController();
        fr.getuser(user);
